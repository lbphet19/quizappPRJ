...
public class QuizController {
	...
	public ResponseEntity<Object> getCatTest(){
		...
		List<CategoryWithQuiz> listResult = catPage.getContent().stream().map(cate -> 
			new CategoryWithQuiz(cate.getId(),cate.getCategoryName())).collect(Collectors.toList());
		for(CategoryWithQuiz e: listResult) {
			Page<Quiz> quizPage = this.quizRepo.getQuizByCatId(e.getCatId(), pageableQuiz);
			List<QuizCatDTO> quizContent = quizPage.getContent().stream()
	...
	public ResponseEntity<Page<Quiz>> searchWithPagination(@RequestParam(name = "sortD", required = false, defaultValue = "2" ) Integer sortD,
