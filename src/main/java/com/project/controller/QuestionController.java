...
public class QuestionController {
	...
	public ResponseEntity<List<Question>> getCorrectAnswers(@PathVariable(name = "quizId") int quizId) {
		List<Question> list = questionRepo.getCorrectAnswers(quizId);
		return ResponseEntity.ok(list);
	}

	@GetMapping(value = "/question/testGetAnswers/{quizId}")
	public ResponseEntity<List<Question>> testGetAnswers(@PathVariable(name = "quizId") int quizId) {
	...
	public ResponseEntity<AnswerResponseDTO> postAnswer(@RequestBody AnswerRequestDTO[] answers,
...
//		Stream<AnswerRequestDTO> answerStream = Stream.of(answers);
		List<AnswerRequestDTO> sortedAnswerRequestDTO = Arrays.asList(answers).stream()
				.sorted(Comparator.comparingInt(AnswerRequestDTO::getQuestionId)).collect(Collectors.toList());
		for (int i = 0; i < sortedAnswerRequestDTO.size(); i++) {
			if (this.answerService.equalsIngnoreOrder(sortedAnswerRequestDTO.get(i).getAnswerIds(),
					correctAnswerIds.get(i))) {
				CorrectAnswerResponseDTO res = new CorrectAnswerResponseDTO(true, correctAnswerIds.get(i),
						sortedAnswerRequestDTO.get(i).getQuestionId());
				correctAnswerResponse.add(res);
				totalScore++;
			} else {
				CorrectAnswerResponseDTO res = new CorrectAnswerResponseDTO(false, correctAnswerIds.get(i),
						sortedAnswerRequestDTO.get(i).getQuestionId());
				correctAnswerResponse.add(res);
			}
		}
		...
		QuizScore quizScore = new QuizScore(this.quizRepo.findById(quizId).get(),
				userRepo.findById(userDetails.getId()).get(), Date.valueOf(LocalDate.now()), score);
	...
	public ResponseEntity<AnswerResponseDTO> postAnswerTest(@RequestBody AnswerRequestDTO[] answers,
			@PathVariable(name = "quizId") int quizId) {
		int totalScore = 0;
		List<Question> correctAnswers = this.questionRepo.getCorrectAnswers(quizId);
		List<List<Integer>> correctAnswerIds = this.questionService.getAnswerIds(correctAnswers);
